...
def test_resize_and_output(tmpdir):
    ...
    st = Stream()
    st.add_resource(ImageResource(props={'src': os.path.join(tmpdir, 'img1.jpg')}))
    st.add_resource(ImageResource(props={'src': os.path.join(tmpdir, 'img2.jpg')}))
    st.add_resource(FilesystemResource(props={'src': os.path.join(tmpdir, 'img3.jpg')}))
    assert 2 == len(st.get_resources('Img:*'))
    assert 1 == len(st.get_resources('FS:*'))

    st.append_node(ImageResizeNode(props={'target_w': '300'}))
    st.append_node(
        ImageOutputNode(props={'output_file': os.path.join(tmpdir, '{img.src.basename}_test.{img.src.extension}')}))

    assert st.flow()

    for i in range(1, 4):
        assert os.path.exists(os.path.join(tmpdir, "img{0}_test.jpg".format(i)))
        imgtmp = Image.open(os.path.join(tmpdir, "img{0}_test.jpg".format(i)))
